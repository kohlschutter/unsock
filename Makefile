...
$(LIBUNSOCK): test-ld src/unsock.c src/main.c
	$(CC) $(CFLAGS) -D_GNU_SOURCE -fvisibility=hidden \
		...
		-o $(LIBUNSOCK).$(LIBUNSOCK_API_VER_MAJOR).$(LIBUNSOCK_API_VER_MINOR).$(LIBUNSOCK_API_VER_PATCH) \
		src/unsock.c src/main.c \
		-ldl
	ln -sf $(LIBUNSOCK).$(LIBUNSOCK_API_VER_MAJOR) $(LIBUNSOCK)
	ln -sf $(LIBUNSOCK).$(LIBUNSOCK_API_VER) $(LIBUNSOCK).$(LIBUNSOCK_API_VER_MAJOR)

install: $(LIBUNSOCK)
	@echo "Installing to $(LIBUNSOCK_INSTALL_DIR)"
	@mkdir -p $(LIBUNSOCK_INSTALL_DIR)
	@cp -av $(LIBUNSOCK) $(LIBUNSOCK).$(LIBUNSOCK_API_VER_MAJOR) $(LIBUNSOCK).$(LIBUNSOCK_API_VER) $(LIBUNSOCK_INSTALL_DIR)/
